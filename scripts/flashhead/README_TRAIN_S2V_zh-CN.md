...
```bash
pip install Pillow einops safetensors timm tomesd librosa pyloudnorm soundfile "torch>=2.1.2" torchdiffeq torchsde decord datasets numpy scikit-image
...
如果原始视频还没有裁剪成人脸稳定片段，可以先运行预处理脚本。脚本会检测人脸、切分稳定片段、裁剪并重采样视频，同时导出 16 kHz 单声道音频和训练用 metadata。
...
预处理完成后，metadata 默认写到 `datasets/flashhead_v2v/metadata.json`。其中 `file_path` 和 `audio_path` 都是相对 `output_dir` 的路径，不会记录本机绝对目录：

```bash
export DATASET_NAME="datasets/flashhead_v2v"
export DATASET_META_NAME="datasets/flashhead_v2v/metadata.json"
```
