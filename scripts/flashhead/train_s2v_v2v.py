...
def parse_args():
    ...
    # V2V training arguments
    parser.add_argument("--v2v_prob", type=float, default=0.8,
                        help="Probability of using V2V mode (source-video keyframe block as y). Remainder uses S2V fallback.")
    parser.add_argument("--keep_heads_max", type=int, default=2,
                        help="Max number of clean leading latent frames to keep (temporal context).")
    parser.add_argument("--spatial_margin_max", type=int, default=2,
                        help="Max spatial border margin in latent pixels to keep clean.")
    parser.add_argument("--audio_dropout_prob", type=float, default=0.15,
                        help="Probability of zeroing audio features (implicit CFG).")
...
def main():
    ...
    # Bell-shaped timestep weights for FlowMatch training.
    _ts = noise_scheduler.timesteps.float()
    _n = len(_ts)
    _y = torch.exp(-2 * ((_ts - _n / 2) / _n) ** 2)
    _y = _y - _y.min()
    bsmntw_weights = _y * (_n / _y.sum())
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                # Extra ref frames: apply same spatial transform
                erf = torch.from_numpy(example["extra_ref_frames"]).permute(0, 3, 1, 2).contiguous()
                erf = erf / 255.
                new_examples["extra_ref_frames"].append(transform(erf))
                new_examples["n_ref"].append(int(example.get("n_ref", erf.shape[0])))
            ...
            new_examples["extra_ref_frames"] = torch.stack([example for example in new_examples["extra_ref_frames"]])
            new_examples["n_ref"] = torch.tensor(new_examples["n_ref"], dtype=torch.long)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                extra_ref_frames = batch["extra_ref_frames"].to(weight_dtype)
                clip_idx = batch["clip_idx"]
                audio = batch["audio"]
                sample_rate = batch["sample_rate"]
                fps = batch["fps"]
                n_ref_tensor = batch["n_ref"] if isinstance(batch["n_ref"], torch.Tensor) else torch.tensor(batch["n_ref"], dtype=torch.long)
                ...
                if args.auto_tile_batch_size and args.training_with_video_token_length and zero_stage != 3:
                    if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                        ...
                        extra_ref_frames = torch.tile(extra_ref_frames, (4, 1, 1, 1, 1))
                        clip_idx = torch.tile(clip_idx, (4,))
                        audio = torch.tile(audio, (4, 1))
                        sample_rate = sample_rate * 4
                        fps = fps * 4
                        n_ref_tensor = torch.tile(n_ref_tensor, (4,))
                        ...
                        extra_ref_frames = torch.tile(extra_ref_frames, (2, 1, 1, 1, 1))
                        clip_idx = torch.tile(clip_idx, (2,))
                        audio = torch.tile(audio, (2, 1))
                        sample_rate = sample_rate * 2
                        fps = fps * 2
                        n_ref_tensor = torch.tile(n_ref_tensor, (2,))
                ...
                with torch.no_grad():
                    ...
                    # Prepare y: V2V uses source-video keyframe blocks; S2V fallback uses ref_image + zeros.
                    def _batch_encode_vae_raw(pv_bctfhw):
                        """Encode pixel values already in [B, C, F, H, W] layout."""
                        bs = args.vae_mini_batch
                        parts = []
                        for i in range(0, pv_bctfhw.shape[0], bs):
                            parts.append(vae.encode(pv_bctfhw[i:i+bs])[0].sample())
                        return torch.cat(parts, dim=0)

                    use_v2v = random.random() < args.v2v_prob
                    if use_v2v:
                        # V2V mode: y = source-video keyframe block pattern.
                        # [frame0×9, ref1×8, ref2×8, ref3×8, ref4×8] for 41 frames.
                        # frame0 is the first frame of the current clip.
                        source_pixels = rearrange(pixel_values, "b f c h w -> b c f h w").clone()
                        frame0 = source_pixels[:, :, :1]  # [B,C,1,H,W]

                        erf = extra_ref_frames.to(dtype=weight_dtype, device=accelerator.device)
                        erf = rearrange(erf, "b f c h w -> b c f h w")
                        ref_slots = [erf[:, :, i:i+1] for i in range(min(4, erf.shape[2]))]
                        while len(ref_slots) < 4:
                            ref_slots.append(ref_slots[-1])

                        block_frame0 = frame0.repeat(1, 1, 9, 1, 1)
                        block_parts = [block_frame0]
                        for rs in ref_slots:
                            block_parts.append(rs.repeat(1, 1, 8, 1, 1))
                        one_block = torch.cat(block_parts, dim=2)  # 41
                        if num_frames <= 41:
                            cond_pixels = one_block[:, :, :num_frames]
                        else:
                            cond_pixels = torch.cat([one_block, one_block[:, :, :num_frames - 41]], dim=2)

                        cond_pixels = cond_pixels.to(dtype=weight_dtype, device=accelerator.device)
                        y = _batch_encode_vae_raw(cond_pixels).to(weight_dtype)
                    else:
                        # S2V fallback: ref_image first frame + zeros
                        cond_video_pixels = torch.zeros(bsz, 3, num_frames, height, width, dtype=weight_dtype, device=accelerator.device)
                        cond_video_pixels[:, :, :1] = rearrange(ref_pixel_values, "b f c h w -> b c f h w")[:, :, :1]
                ...
                with torch.no_grad():
                    ...
                    # Build audio window indices: [-2, -1, 0, 1, 2] for audio_window=5.
                    ...
                    # Audio dropout for implicit CFG
                    if random.random() < args.audio_dropout_prob:
                ...
                # V2V noise injection with keep_heads and spatial margin
                sigmas = get_sigmas(timesteps, n_dim=latents.ndim, dtype=latents.dtype)

                # Keep clean leading latent frames as temporal context, biased toward longer context.
                r = random.random()
                if r < 0.1:
                    keep_heads = 0
                elif r < 0.3:
                    ...
                    keep_heads = min(3, args.keep_heads_max)
                keep_heads = min(keep_heads, lat_f - 1)  # at least 1 frame must be noised

                # Preserve a clean spatial border to stabilize V2V reconstruction.
                m = random.choices([0, 1, 2], weights=[0.1, 0.3, 0.6], k=1)[0]
                m = min(m, args.spatial_margin_max, lat_h // 2 - 1, lat_w // 2 - 1)
                hs = slice(m, lat_h - m) if m > 0 else slice(None)
                ws = slice(m, lat_w - m) if m > 0 else slice(None)

                # Only noise the interior of non-head frames
                x_core = latents[:, :, keep_heads:, hs, ws].clone()
                noise_core = torch.randn(x_core.size(), device=x_core.device, generator=torch_rng, dtype=weight_dtype)
                noisy_core = (1.0 - sigmas) * x_core + sigmas * noise_core

                # Assemble full noisy_latents: start with clean latents, inject noise in core region
                noisy_latents = latents.clone()
                noisy_latents[:, :, keep_heads:, hs, ws] = noisy_core

                # If no keep_heads, still inject motion_latents at the beginning
                if keep_heads == 0:
                    noisy_latents[:, :, :lat_motion_frames] = motion_latents[:, :, :lat_motion_frames]

                # Target only for the noised core region
                target_core = noise_core - x_core
                ...
                # Loss only on the noised core region.
                noise_pred_core = noise_pred[:, :, keep_heads:, hs, ws]
                loss_per_sample = ((noise_pred_core.float() - target_core.float()) ** 2)
                loss_per_sample = loss_per_sample.mean(dim=tuple(range(1, loss_per_sample.ndim)))
                loss_weights = bsmntw_weights[indices].float().to(loss_per_sample.device)
                loss = (loss_per_sample * loss_weights).mean()

                if args.motion_sub_loss and noise_pred_core.size()[2] > 2:
                    gt_sub_noise = noise_pred_core[:, :, 1:].float() - noise_pred_core[:, :, :-1].float()
                    pre_sub_noise = target_core[:, :, 1:].float() - target_core[:, :, :-1].float()
